...
#[derive(Debug, Snafu)]
enum OdbcError {
    #[snafu(display("ODBC error: {source}"))]
    OdbcError { source: odbc_api::Error },
    #[snafu(display("File IO error: {source}"))]
    IoError { source: std::io::Error },
    #[snafu(display("Batch error: {source}"))]
    ClosedError { source: ClosedError },
}
...
impl DerefMut for OdbcSchedule {
...
fn load_params(path: &str, columns_order: Option<&Vec<String>>) -> Option<Vec<VarCharBox>> {
    ...
    let map = serde_json::from_reader(reader).unwrap_or(HashMap::<String, Value>::default());
    ...
    let params = columns_order
        .into_iter()
        .filter_map(|col| {
            let Some(value) = map.get(col) else {
                return None;
            };
...
fn map_value(data_type: &odbc_api::DataType, value: Option<&[u8]>, tz: &Tz) -> Value {
    match data_type {
        ...
        odbc_api::DataType::BigInt | // TODO: will be support
        ...
        odbc_api::DataType::Timestamp { .. } => {
            ...
            datetime.map(|dt| Value::Timestamp(dt)).unwrap_or(Value::Null)
        }
        odbc_api::DataType::Time { .. } => {
            ...
            std::str::from_utf8(value).ok()
                ...
                .map(|time| {
                    ...
                    Value::Timestamp(DateTime::<Tz>::from_naive_utc_and_offset(datetime, tz).to_utc())
                })
